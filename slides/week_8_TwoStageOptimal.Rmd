...
- You need a guess at MSB and MSW
  ...
  -  $MSB = S^2_t/M$ (how to cluster totals vary?)
  - $MSW = \sum_{i}^{N} S^2_i/N$ (within cluster variation?)

--

- HW #7: How to compute MSB and MSW from guesses of $R^2_a$ and $S^2$
...
- Previous study:  [One-stage example](https://kstclair.github.io/stat-260/slides/week_7_OneStagevsSRS.pdf)
...
## Example: Dorms  with $R^2_a = 0.334$
...
ggplot(mydata_grid, aes(x = m, y = se, color = cost)) + 
  ...
  labs(title=TeX("SE as a function of m for cost of 300 minutes and $R^2_a = 0.334$")) + 
  theme_minimal() + 
  ...
  scale_y_continuous(limits = c(0.14,.30))
...
ggplot(mydata_grid, aes(x = m, y = se, color = cost)) + 
  geom_line(aes(linetype = cost)) + 
  geom_point(data = mydata, aes(x=m, y = se, color = cost)) +
  labs(title=TeX("SE as a function of m for cost of 300 minutes and $R^2_a = 0.7$")) + 
  theme_minimal() + 
  theme(axis.text=element_text(size=14),
        axis.title=element_text(size=16,face="bold"), 
        legend.text = element_text(size=16),
